...
def crearVentanaDatosUsuario():
    ...
    layout = [
        ...
        [sg.Image(filename="logo_tec.png")], # Agregar imagen
...
#Funcion para ejecutar la ventanade estadisticas
def ejecutarVentanaPrincipal(ventana):
    while True: #Ciclo que acaba cuando se cierra la ventana o se da click en salir
        ...
        elif evento == "Tipo de Servicio":
            webbrowser.open("https://docs.google.com/spreadsheets/d/1Y5YkTOIySihunjce5uqGuu9buEbTHkusGit3ItCaTXQ/edit#gid=0")
        elif evento == "Contenido Multimedia":
            webbrowser.open("https://www.bancomundial.org/es/topic/education/overview")
            
    ventana.close()

#Funcon para crear la ventana de estadisticas
def crearVentanaStats():
    datos=[] #Lista para guardar los registros y procesarlos posteriormente
    archivoCsv = "usuarios.csv" # Variable del archivo
    with open(archivoCsv, "r") as file: #Iteracion sobre todos los registros del archivo
        csvReader = csv.reader(file)
        for row in csvReader: #En este ciclo se agregan los registros a la lista
            datos.append(row)

    numRegistros = len(datos) #Se obtiene la longitud de la lista, osea el numero de registros

    #Aqui se obtiene el promedio de la edad
    edades = [int(row[1]) for row in datos]# se guarda en la variable los valores de la segunda columna que corresponde a las edaes
    meanAge = int(sum(edades) / numRegistros) #Se saca el promedio de las edades
    #Aqui se cuentan las intancias de cada motivo
    motivos = [row[3] for row in datos] #Se guardan los valores de la 4ta columna
    contador = Counter(motivos)#Se cuentan los valores de cada motivo
    layout = [
        [sg.Image(filename="logo_tec.png")], # Agregar imagen
        [sg.Text("Estadísticas", font=("Helvetica", 16))], #Titulo de la ventana
        [sg.Text("Datos Registrados: "+str(numRegistros), font=("Helvetica", 12))], 
        [sg.Text("Edad Promedio: "+ str(meanAge), font=("Helvetica", 12))],
        [sg.Text("Motivo mas frecuente: "+ contador.most_common(1)[0][0], font=("Helvetica", 12))], #Se agrega el comando de most_common para que te saque el valor mas comun del motivo
        [sg.Text("Categoria de Servicios Educativos", font=("Helvetica", 12))],
        [sg.Text("Educativo: 8", font=("Helvetica", 12))],
        [sg.Text("Deportivo: 3", font=("Helvetica", 12))],
        [sg.Text("Culturales: 4", font=("Helvetica", 12))],
        [sg.Button("Salir")] #Boton del evento Iniciar Sesion
    ]
    ventana = sg.Window("Estadísticas", layout) #Se asiga el layour en la ventana
    return ventana #Regresa la ventana


def ejecutarVentanaStats(ventana):
    while True: #Ciclo que acaba cuando se cierra la ventana o se da click en salir
    ...
    while True: #Itera hastsa que se cierre la ventana
        ...
        if evento == "Iniciar sesión": #Una vez que inicia sesion se corre este codigo
            ...
            if usuario == "tec2023" and contraseña == "tec2023": #Valida que el usuario y la contraseña sean "tec2023" 
                ...
                ventanaEstadisicas = crearVentanaStats()
                ejecutarVentanaStats(ventanaEstadisicas)
